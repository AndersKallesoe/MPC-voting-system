...
pub fn protocol_server(mainaddr: SocketAddrV4){
    ...
    thread::spawn(
        move
        ||{listen_for_clients(client_listener,arc_shares)}
    );
...
fn listen_for_clients(client_listener: TcpListener,arc_shares: Arc<Mutex<Vec<i64>>>){

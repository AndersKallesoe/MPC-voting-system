...
fn _test_protocol(protocol: Protocol, corrupt: Vec<u8>, times: i64){
    ...
    for _i in 0..times{
        let c = corrupt.clone();
        let ( mut result,results) = run_protocol(protocol,c, false);
...
/*
    Demonstrate
*/
pub fn _demonstrate_additive(){
    let additive_protocol_1 = Protocol{prime: 29, servers: 2, voters: 20, protocol: ProtocolType::Additive};
    run_and_report(additive_protocol_1,vec![], true);   
    let additive_protocol_2 = Protocol{prime: 113, servers: 5, voters: 100, protocol: ProtocolType::Additive};
    run_and_report(additive_protocol_2,vec![], true);   
}
pub fn _demonstrate_shamir(){
    let shamir_protocol_1 = Protocol{prime: 29, servers: 2, voters: 20, protocol: ProtocolType::Shamir};
    run_and_report(shamir_protocol_1,vec![], true);
    let shamir_protocol_2 = Protocol{prime: 113, servers: 5, voters: 10, protocol: ProtocolType::Shamir};
    run_and_report(shamir_protocol_2,vec![], true);
}
pub fn _demonstrate_fault_detection(){
    let fault_detection_protocol_1 = Protocol{prime: 29, servers: 3, voters: 20, protocol: ProtocolType::ShamirFaultDetection};
    run_and_report(fault_detection_protocol_1,vec![1], true);
    run_and_report(fault_detection_protocol_1,vec![], true);
    let fault_detection_protocol_2 = Protocol{prime: 113, servers: 5, voters: 100, protocol: ProtocolType::ShamirFaultDetection};
    run_and_report(fault_detection_protocol_2,vec![1,4], true);
    run_and_report(fault_detection_protocol_2,vec![], true);
}
pub fn _demonstrate_error_correction(){
    let error_correction_protocol_1 = Protocol{prime: 29, servers: 4, voters: 20, protocol: ProtocolType::ShamirErrorCorrection};
    run_and_report(error_correction_protocol_1, vec![1], true);
    let error_correction_protocol_2 = Protocol{prime: 29, servers: 7, voters: 20, protocol: ProtocolType::ShamirErrorCorrection};
    run_and_report(error_correction_protocol_2, vec![1,6], true);
    run_and_report(error_correction_protocol_2, vec![], true);
    run_and_report(error_correction_protocol_2, vec![1], true);
    run_and_report(error_correction_protocol_2, vec![1,4,6], true);
}

pub fn run_and_report(protocol: Protocol, corruptions: Vec<u8>, print: bool){
    let (result, results) = run_protocol(protocol, corruptions.clone(), print);
...
pub fn _benchmark_protocol(protocol: Protocol, corrupt: Vec<u8>, runs: i64){
    line();
    ...
    for _i in 0..runs{
        let now = Instant::now();
        let c = corrupt.clone();
        let (result,res) = run_protocol(protocol,c, false);
